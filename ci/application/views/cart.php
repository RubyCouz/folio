            ...
            foreach ($this->session->cart as $prod) {
                ...
                <div class="row" id="<?= $prod['pro_id'] ?>">
                    <div class="col s1 valign-wrapper">
                        <img src="<?= base_url('assets/img/' . $prod['pro_id'] . '.' . $prod['pro_photo']) ?>" alt=""
                             class="circle responsive-img" title="Photo d'un article"
                             alt="Photo de <?= $prod['pro_libelle'] ?>">
                    </div>
                    <div class="col s2">
                    ...
                    <div class="col s2">
                        <form class="row">
                            <input type="hidden" name="pro_qte" value="<?= $prod['pro_qte'] ?>">
                            <input type="hidden" name="pro_id" value="<?= $prod['pro_id'] ?>">
                            <div class="col s12 modify">
                                <button type="button" class="waves-effect waves-light btn tiny material-icons add"
                                        value="<?= $prod['pro_id'] ?>">add
                                </button>
                            </div>
                            <div class="col s12 modify">
                                <button type="button" class="waves-effect waves-light btn tiny material-icons remove"
                                        value="<?= $prod['pro_id'] ?>">remove
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="col s3">
                        <button class="waves-effect waves-light btn red del" id="delProduct"
                                value="<?= $prod['pro_id'] ?>">Supprimer
                        </button>
                    </div>
                </div>
                <?php
